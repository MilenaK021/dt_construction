...
export default function TaskList({ projectId }) {
  ...
  const fetchTasks = useCallback((silent = false) => {
    ...
    getTasks(projectId)
      .then(res => { setTasks(res.data.tasks); setLastSync(new Date()) })
    ...
    <div>
      <div className="tasklist-header">
        ...
        <div className="tasklist-sync">
          ...
          <span className="sync-label">{lastSync ? `Synced ${fmtTime(lastSync)}` : ''}</span>
          <button className="sync-btn" onClick={() => fetchTasks(true)} disabled={refreshing}>
            ↻ Refresh
          </button>
        </div>
      </div>
